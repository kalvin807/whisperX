...
class SubtitlesProcessor:
            ...
            if advanced_splitting:
                ...
                for i, word in enumerate(words):
                    if "start" not in word or "end" not in word:
                        self.estimate_timestamp_for_word(
                            words, i, next_segment_start_time
                        )
    ...
    def determine_advanced_split_points(self, segment, next_segment_start_time=None):
        ...
        total_char_count = sum(
            len(word["word"]) if isinstance(word, dict) else len(word) + add_space
            for word in words
        )
        ...
        for split_point in split_points:
            ...
            if isinstance(fragment_words[0], dict):
                ...
                next_start_time_for_word = (
                    words[split_point + 1]["start"]
                    if split_point + 1 < len(words)
                    else None
                )
                if (
                    next_start_time_for_word
                    and (next_start_time_for_word - end_time) <= 0.8
                ):
